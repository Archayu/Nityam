...
module.exports = {
  ...
  run: async (interaction, client) => {
    ...
    const guildSettings = await AntinukeGuildSettings.findOne({
      guildId: interaction.guild.id,
    });
    if (!guildSettings) {
      return interaction.editReply({
        content: "Antinuke is not enabled for this guild!",
        ephemeral: true,
      });
    }
    ...
    if (category === "owner") {
      ...
      if (!whitelist || whitelist.length === 0)
        return interaction.editReply({
          content: "No users are in owner whitelist",
          ephemeral: true,
        });
      ...
      if (!whitelist || whitelist.length === 0) {
        return interaction.editReply({
          content: "No users are whitelisted for this antinuke category!",
          ephemeral: true,
        });
      }
