...
module.exports = async (client) => {
    ...
    client.createExSetup = async function (interaction) {
        const db = new Database("./Models/Json/setup.json", { databaseInObject: true });
    ...
    client.createEVoice = async function (interaction) {
        const db = new Database("./Models/Json/voice.json", { databaseInObject: true });
        await db.set(interaction.guild.id, {
            voice_enable: true,
        });
    }
    ...
    client.createExVoice = async function (interaction) {
        const db = new Database("./Models/Json/voice.json", { databaseInObject: true });
        const database =  db.has(interaction.guild.id);
        if (!database) {
            await db.set(interaction.guild.id, {
                voice_enable: false,
            });
    ...
    client.createPlay = async function (interaction, message) {
        const db = new Database("./Models/Json/message.json", { databaseInObject: true });
        await db.set(interaction.guild.id, {
    ...
    client.addCount = async function (cmdname) {
        const db = new Database("./Models/Json/stats.json", { databaseInObject: true });
        await db.add({ key: cmdname, value: 1 });
    }

    client.addChart = async function (id) {
        const db = new Database("./Models/Json/chart.json", { databaseInObject: true });
